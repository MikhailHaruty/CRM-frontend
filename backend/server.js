...
// importing default Node.js libraries
...
// URI prefix for all methods
const URI_PREFIX = '/api/clients';

/**
 * Error class, send response with error's description
 */
...
/**
 * Async reading request'body as JSON
 * @param {Object} req - HTTP's object
 * @throws {ApiError} Invalid data in args
 * @returns {Object} Object, created from request
 */
...
/**
 * Checking valid data and Creating correct object with client
...
module.exports = createServer(async (req, res) => {
  ...
  .on('listening', () => {
    if (process.env.NODE_ENV !== 'test') {
      console.log(`Server is launched. You can use if by address: http://localhost:${PORT}`);
      console.log('Press CTRL + C to stop server');
      console.log('Available methods:');
      console.log(`GET ${URI_PREFIX} - get clients`);
      console.log(`POST ${URI_PREFIX} - create client, send to the request's body: { name: string, surname: string, lastName?: string, contacts?: object[] }`);
      console.log(`\tcontacts - the array of objects with contacts, like { type: string, value: string }`);
      console.log(`GET ${URI_PREFIX}/{id} - get client by ID`);
      console.log(`PATCH ${URI_PREFIX}/{id} - change client's data by ID, send to the request's body: { name?: string, surname?: string, lastName?: string, contacts?: object[] }`);
      console.log(`\tcontacts - массив объектов контактов вида { type: string, value: string }`);
      console.log(`DELETE ${URI_PREFIX}/{id} - delete client by ID`);
    }
  })
